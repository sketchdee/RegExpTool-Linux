...
public class Tools {
    ...
    public static LinkedList<String> ConsoleExec() {
        ...
        try {
            ...
            if(System.getProperty("os.name").toLowerCase(Locale.ROOT).startsWith("win"))
                bufferedReader = new BufferedReader(new InputStreamReader(process.getInputStream(), "GBK")); //控制台得到的是GBK流
    ...
    //linux 会以一些特定的-l等表示动作，这样就不用刻意的去在意顺序了，这个有空再总结吧，现在没必要，自己写自己用。
